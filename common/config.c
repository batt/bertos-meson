...
#include <common/dli.h> // include before stdio.h otherwise there will be clash with clearerr
...
void config_load(ConfigMetadata *cfg)
{
	...
	for (size_t i = 0; i < cfg->tbl_size; ++i)
	{
		...
		if (!dli_get(e->name, e->default_val, input_string, sizeof(input_string)))
...
SetPRetVals config_setInt(const struct ConfigEntry *e, char *val, bool use_default)
{
	...
	if ((errno == ERANGE && (conv == LONG_MAX || conv == LONG_MIN)) || (errno != 0 && conv == 0) || (endptr == val))
	{
		if (use_default)
		{
