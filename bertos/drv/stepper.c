...
{
	return (motor->cfg->homeSensorIndex < NUM_HOME_SENSORS) ? hw_home_sensor_read(motor->cfg->homeSensorIndex) : bld_hw_sensor_read(motor->cfg->homeSensorIndex - NUM_HOME_SENSORS);
}
...
static bool stepper_checkHomeErrors(struct Stepper *motor)
{
	...
	if (motor->enableCheckHome == MOTOR_HOMESENSOR_INCHECK && home && (!motor->stepCircular || motor->step < motor->stepCircular / 2))
		/*
...
static enum StepperState FAST_FUNC FSM_run(struct Stepper *motor)
{
	...
	if (!stepper_checkHomeErrors(motor) && !motor->cfg->flags.noHomeSensor)
		return MSTS_ERROR;
	...
	if (motor->rampStep < 0)
	{
		 ...
		 */
		if (motor->enableCheckHome != MOTOR_HOMESENSOR_NOCHECK && !motor->cfg->flags.noHomeSensor)
			return MSTS_ERROR;
...
static enum StepperState FSM_prerun(struct Stepper *motor)
{
	    ...
	    (motor->stepToReach == STEPS_INFINITE_NEGATIVE))
	{
		...
		if (motor->stepToReach == STEPS_INFINITE_NEGATIVE)
			motor->dir = DIR_POSITIVE;
...
static enum StepperState FSM_preinit(struct Stepper *motor)
{
	 ...
	 *
	 */
	if (!motor->cfg->flags.noHomeSensor && stepper_readHome(motor))
	{
...
static enum StepperState FSM_init(struct Stepper *motor)
{
	...
	{
		if (timer_clock() - motor->startTime > ms_to_ticks(motor->cfg->timeoutHome))
		{
			// If no home sensor is installed, assume we are in home after timeout.
			if (motor->cfg->flags.noHomeSensor)
			{
				motor->step = 0;
				return MSTS_PREIDLE;
			}
...
int16_t stepper_scaleSteps(struct Stepper *motor, int16_t dir)
{
	...
	if (!motor->stepCircular)
		return 0;

	// to be sure ....
	while (motor->step > motor->stepCircular)
		motor->step -= motor->stepCircular;
...
int16_t stepper_move(struct Stepper *motor, int16_t steps, uint16_t speed, int16_t deafstep)
{
	...
	{
		LOG_ERR("Runtime ramp error: (max=%x, min=%x)\n", motor->cfg->ramp.clocksMaxWL, motor->cfg->ramp.clocksMinWL);
