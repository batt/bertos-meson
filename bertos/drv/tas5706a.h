...
#if COMPILER_C99
	#define tas5706a_init(...)                             \
		PP_CAT(tas5706a_init##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define tas5706a_setLowPower(...)                             \
		PP_CAT(tas5706a_setLowPower##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define tas5706a_setVolume(...)                             \
		PP_CAT(tas5706a_setVolume##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
#else
	#define tas5706a_init(args...)                  \
		PP_CAT(tas5706a_init##_, COUNT_PARMS(args)) \
		(args)
	#define tas5706a_setLowPower(args...)                  \
		PP_CAT(tas5706a_setLowPower##_, COUNT_PARMS(args)) \
		(args)
	#define tas5706a_setVolume(args...)                  \
		PP_CAT(tas5706a_setVolume##_, COUNT_PARMS(args)) \
		(args)
#endif
