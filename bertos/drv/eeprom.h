...
#if COMPILER_C99
	#define eeprom_init(...)                             \
		PP_CAT(eeprom_init##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define eeprom_verify(...)                             \
		PP_CAT(eeprom_verify##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
#else
	#define eeprom_init(args...)                  \
		PP_CAT(eeprom_init##_, COUNT_PARMS(args)) \
		(args)
	#define eeprom_verify(args...)                  \
		PP_CAT(eeprom_verify##_, COUNT_PARMS(args)) \
		(args)
#endif
