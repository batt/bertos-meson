...
#include <cpu/attr.h>
...
#if COMPILER_C99
	#define flash_init(...)                           \
		PP_CAT(flash_init_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
#else
	/**
	 ...
	 */
	#define flash_init(args...)                \
		PP_CAT(flash_init_, COUNT_PARMS(args)) \
		(args)
#endif
 ...
 */
#define flash_init_2(fls, flags) (flags & FLASH_UNBUFFERED) ? flash_hw_initUnbuffered(fls, flags) : flash_hw_init(fls, flags)
