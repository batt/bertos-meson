...
#include <drv/timer.h>
		...
		#define KDBG_WRITE_CHAR(c)                       \
			...
			{                                            \
				char __c = (c);                          \
				write(STDERR_FILENO, &__c, sizeof(__c)); \
			} while (0)
...
static klogger_stack[KLOGGER_STACK_SIZE];
		#endif
...
int PGM_FUNC(__bassert)(const char *PGM_ATTR cond, const char *PGM_ATTR file, int line)
{
	...
	#endif
	BREAKPOINT;
	#if CPU_CM3_STM32F2 | CPU_CM3_STM32F1
