...
/* These can be macros in stdio.h, undef them */
#undef getc
#undef putc
...
/**
  ...
  */
#if COMPILER_C99
	#define i2c_init(...)                             \
		PP_CAT(i2c_init##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define i2c_start_w(...)                             \
		PP_CAT(i2c_start_w##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define i2c_start_r(...)                             \
		PP_CAT(i2c_start_r##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
#else
	/**
	 ...
	 * legacy code.
	 */
	#define i2c_init(args...)                  \
		PP_CAT(i2c_init##_, COUNT_PARMS(args)) \
		(args)
	 ...
	 */
	#define i2c_start_w(args...)                  \
		PP_CAT(i2c_start_w##_, COUNT_PARMS(args)) \
		(args)
	 ...
	 * \note Use the version with 4 parameters, the others are only for legacy code
	 */
	#define i2c_start_r(args...)                  \
		PP_CAT(i2c_start_r##_, COUNT_PARMS(args)) \
		(args)
#endif
/**\}*/
 ...
 */
#define i2c_init_3(i2c, dev, clock) ((((dev) >= I2C_BITBANG0) | ((dev) == I2C_BITBANG_OLD)) ? i2c_hw_bitbangInit((i2c), (dev)) : i2c_hw_init((i2c), (dev), (clock)))
/**@}*/
/**\}*/ // i2c_api
