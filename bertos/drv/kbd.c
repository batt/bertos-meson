...
static void kbd_poll(void)
{
	...
	if (key != current_key)
	{
		/* Get only the new keys */
		keymask_t new_key = (key ^ current_key) & key;
...
void kbd_addHandler(struct KbdHandler *handler)
{
	...
	list = (handler->flags & KHF_RAWKEYS) ? &kbd_rawHandlers : &kbd_handlers;
...
static keymask_t kbd_debHandlerFunc(keymask_t key)
{
	...
	else if ((new_key != debounce_key) && (now - debounce_time > ms_to_ticks(KBD_DEBOUNCE_TIME)))
	{
...
static keymask_t kbd_lngHandlerFunc(keymask_t key)
{
	...
	if (key & K_LNG_MASK)
	{
		...
		{
			key |= K_LONG;
			prev_key = key & (K_LNG_MASK | K_LONG);
		}
		else
		{
			/* Filter long pression keys until they are released */
			prev_key = key & (K_LNG_MASK | K_LONG);
			key &= ~K_LNG_MASK;
		}
	}
...
#if CONFIG_KBD_FALLINGEDGE
static KbdHandler kbd_fallingEdgeHandler; /**< Falling edge keys handler */

/**
 * Handle keyboard falling edge
 * Only send a key when it is de-pressed.
 */
static keymask_t kbd_fallingEdgeHandlerFunc(keymask_t key)
{
	static keymask_t prev_key;

	keymask_t new_key = (prev_key ^ key) & prev_key;
	prev_key = key;
...
#if CONFIG_KBD_FALLINGEDGE
	kbd_fallingEdgeHandler.hook = kbd_fallingEdgeHandlerFunc;
	kbd_fallingEdgeHandler.pri = 85;
	kbd_fallingEdgeHandler.flags = KHF_RAWKEYS;
	kbd_addHandler(&kbd_fallingEdgeHandler);
#endif
