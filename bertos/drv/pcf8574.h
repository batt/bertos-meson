...
#if COMPILER_C99
	#define pcf8574_init(...)                             \
		PP_CAT(pcf8574_init##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define pcf8574_get(...)                             \
		PP_CAT(pcf8574_get##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
	#define pcf8574_put(...)                             \
		PP_CAT(pcf8574_put##_, COUNT_PARMS(__VA_ARGS__)) \
		(__VA_ARGS__)
#else
	#define pcf8574_init(args...)                  \
		PP_CAT(pcf8574_init##_, COUNT_PARMS(args)) \
		(args)
	#define pcf8574_get(args...)                  \
		PP_CAT(pcf8574_get##_, COUNT_PARMS(args)) \
		(args)
	#define pcf8574_put(args...)                  \
		PP_CAT(pcf8574_put##_, COUNT_PARMS(args)) \
		(args)
#endif
