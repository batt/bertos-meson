...
#define FIFO_END(fifo) &(fifo)->buffer[countof((fifo)->buffer) - 1]

/*
#define ASSERT_VALID_FIFO(fifo) \
	ATOMIC( \
		ASSERT((fifo)->head >= (fifo)->buffer); \
		ASSERT((fifo)->head <= FIFO_END(fifo)); \
		ASSERT((fifo)->tail >= (fifo)->begin); \
		ASSERT((fifo)->tail <= FIFO_END(fifo)); \
	)
*/
 ...
 */
#define FIFO_ISFULL(fb) ((((fb)->head == (fb)->buffer) && ((fb)->tail == FIFO_END(fb))) || ((fb)->tail == (fb)->head - 1))
...
#define FIFO_PUSH(fb, c)                          \
	...
	{                                             \
		*((fb)->tail) = (c);                      \
		if (UNLIKELY((fb)->tail == FIFO_END(fb))) \
			(fb)->tail = (fb)->buffer;            \
		else                                      \
			(fb)->tail++;                         \
	} while (0)
...
#define FIFO_POP(fb)                              \
	({                                            \
		typeof(*(fb)->head) ret;                  \
		if (UNLIKELY((fb)->head == FIFO_END(fb))) \
		{                                         \
			(fb)->head = (fb)->buffer;            \
			ret = *(FIFO_END(fb));                \
		}                                         \
