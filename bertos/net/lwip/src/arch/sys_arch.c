...
err_t sys_mbox_new(sys_mbox_t *mbox, UNUSED_ARG(int, size))
{
	msg_initPort(mbox, event_createGeneric());
...
{
	while (sys_mbox_trypost(mbox, data) != ERR_OK)
		cpu_relax();
}
				...
				void *arg, int stacksize, int prio)
{
	...
	#if !CONFIG_KERN_HEAP
		ASSERT(last_stack < MAX_THREAD_CNT);
		ASSERT(stacksize > 0);
		ASSERT((unsigned)stacksize <= DEFAULT_THREAD_STACKSIZE);
