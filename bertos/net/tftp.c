...
static int checkPacket(TftpSession *ctx, const Tftpframe *frame)
{
	...
	// Check for duplicate WRQ packets

	if (ntohs(frame->hdr.opcode) == TFTP_DATA)
	{
		unsigned short received_block = ntohs(frame->hdr.th_u.block);

		if (received_block > ctx->block + 1)
		{
			LOG_ERR("Error, received block %hd, waiting for %hd\n", ntohs(frame->hdr.th_u.block), ctx->block + 1);
			return CHECK_ERR;
		}
		else if (received_block < ctx->block + 1)
		{
			// Duplicate block
			dup = true;
			//LOG_WARN("Duplicate block %hd, waiting %hd\n", received_block, ctx->block + 1);
		}
		else
		{
			// received_block == ctx->block + 1, OK!
			ctx->block++;
		}
	}
	else if (frame->hdr.opcode == TFTP_WRQ && ctx->block == 0)
	{
		//Duplicate WRQ packet
		dup = true;
	}
	else
	{
		LOG_ERR("Opcode != TFTP_DATA (%hd != %d)\n", ntohs(frame->hdr.opcode), TFTP_DATA);
		return CHECK_ERR;
	}
	...
	ssize_t rc = lwip_sendto(ctx->sock, &ack, sizeof(ack), 0, (struct sockaddr *)&ctx->addr, ctx->addr_len);
	if (rc == sizeof(ack))
		return (dup ? CHECK_DUP : CHECK_OK);

	LOG_ERR("error sending ACK, rc %d, errno %d\n", rc, errno);
	return CHECK_ERR;
}
...
static ssize_t tftp_readPacket(TftpSession *ctx, Tftpframe *frame, mtime_t timeout)
{
	...
	while (1)
	{
		...
		if (rlen > 0)
		{
			int check = checkPacket(ctx, frame);
			if (check == CHECK_OK)
				return rlen;
			else if (check == CHECK_DUP)
				continue;
			else
				return TFTP_ERR;
		}
	...
	if (fds->bytes_available < size)
	{
		...
		if (!fds->is_xfer_end)
		{
			...
			if (rd < 0)
			{
				...
				/* Get actual lwIP error from errno */
				fds->error = errno;
				return 0;
			}
			else
...
static int tftp_close(struct KFile *fd)
{
	...
	if (fds->pending_ack)
	{
		...
		strncpy(err.str, fds->error_msg, sizeof(err.str));
		err.str[sizeof(err.str) - 1] = '\0';
		lwip_sendto(fds->sock, &err, sizeof(err), 0, (struct sockaddr *)&fds->addr, fds->addr_len);
...
void tftp_setErrorMsg(TftpSession *ctx, const char *msg)
{
	strncpy(ctx->error_msg, msg, sizeof(ctx->error_msg));
}

#ifdef _TFTP_CLIENT_NOT_WORKING
/*
 * Initialize a TFTP client connection.
 * Request file \a filename from the server. Since the first ack by the
 * server also contains the first data packet, received data is put
 * into \a buf.
 *
 * \param filename Name of file to be requested.
 * \param buf Data buffer, must be 516 bytes
 * \param size Must be 516
 * \return The number of bytes received from the server if ok, -1 otherwise.
 */
ssize_t tftp_tryConnect(TftpSession *ctx, const char *server_ip, short port, const char *filename, char *buf, size_t size)
{
	unsigned char *cp;
	const unsigned char *cp1 = (const unsigned char *)filename;
	int slen;
	ctx->block = 0;
	...
	sa.sin_family = AF_INET;
	sa.sin_port = htons(port);
	if (inet_aton(server_ip, &sa.sin_addr) == 0)
		LOG_INFO("Error converting server IP address\n");

	// Prepare the transmit buffer for a file request.
	tftp.th_opcode = TFTP_RRQ;
	slen = 2;
	cp = (unsigned char *)tftp.th_u.tu_stuff;
	LOG_INFO("Loading %s\n", cp1);
	do
	{
		*cp = *cp1++;
	...
	memcpy(cp, (const unsigned char *)"octet", 6);
	slen += 6;

	// connect to server
	memset(&ctx->addr, 0, sizeof(ctx->addr));
	LOG_INFO("Sending to %s\n", inet_ntoa(sa.sin_addr));
	if (lwip_sendto(ctx->sock, &tftp, sizeof(tftp), 0, (struct sockaddr *)&sa, slen) == -1)
		return -1;

	// receive answer from server and read data
	ctx->addr_len = sizeof(ctx->addr);
	ssize_t rd = 0;
	if ((rd = lwip_recvfrom(ctx->sock, buf, size, 0, (struct sockaddr *)&ctx->addr, &ctx->addr_len)) > 0)
	{
		LOG_INFO("Received something! Server address: (%s, %d)\n", inet_ntoa(server_addr.sin_addr), ntohs(server_addr.sin_port));
		if (checkPacket(ctx, buf, size) > 0)
			return rd;
	}
	LOG_INFO("Server not responding\n");
	return -1;
}
#endif
