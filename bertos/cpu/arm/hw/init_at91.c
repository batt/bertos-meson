...
#define XTAL_FREQ 18432000UL
...
#if USE_FIXED_PLL
	#if CPU_FREQ != 47232000UL
	    /* Avoid errors on nightly test */
		#if !defined(ARCH_NIGHTTEST) || !(ARCH & ARCH_NIGHTTEST)
			#warning Clock registers set for 47.232MHz operation, revise following code if you want a different clock.
		#endif
	#endif

    /*
	 * With a 18.432MHz cristal, master clock is:
	 * (((18.432 * (PLL_MUL_VAL + 1)) / PLL_DIV_VAL) / AT91MCK_PRES) = 47.232MHz
	 */
	#define PLL_MUL_VAL  40 /**< Real multiplier value is PLL_MUL_VAL + 1! */
	#define PLL_DIV_VAL  8
	...
	#define DIV_MIN (DIV_ROUND(XTAL_FREQ, PLL_IN_MAX) < DIV_HARD_MIN ? DIV_HARD_MIN : DIV_ROUND(XTAL_FREQ, PLL_IN_MAX))

	#define DIV_MAX (DIV_ROUND(XTAL_FREQ, PLL_IN_MIN) > DIV_HARD_MAX ? DIV_HARD_MAX : DIV_ROUND(XTAL_FREQ, PLL_IN_MIN))
...
#define FMCN (CPU_FREQ <= 33333UL ? 0 : (MCN < 0xFF ? MCN : 0xFF))
	...
	/*
         ...
         */
	CKGR_PLLR = ((mul << CKGR_MUL_SHIFT) | (28 << CKGR_PLLCOUNT_SHIFT) | div);
