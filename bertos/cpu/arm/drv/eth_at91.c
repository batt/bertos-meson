 ...
 */
volatile BufDescriptor tx_buf_tab[EMAC_TX_DESCRIPTORS] ALIGNED(8);
...
{
	// PHY read command.
	EMAC_MAN = EMAC_SOF | EMAC_RW_READ | (NIC_PHY_ADDR << EMAC_PHYA_SHIFT) | ((reg << EMAC_REGA_SHIFT) & EMAC_REGA) | EMAC_CODE;
...
{
	// PHY write command.
	EMAC_MAN = EMAC_SOF | EMAC_RW_WRITE | (NIC_PHY_ADDR << EMAC_PHYA_SHIFT) | ((reg << EMAC_REGA_SHIFT) & EMAC_REGA) | EMAC_CODE | val;
...
void eth_setMac(MacAddress mac)
{
	...
	EMAC_SA1L = (mac.addr[3] << 24) | (mac.addr[2] << 16) |
	            (mac.addr[1] << 8) | mac.addr[0];
	EMAC_SA1H = (mac.addr[5] << 8) | mac.addr[4];
}

MacAddress eth_mac(void)
{
	MacAddress mac;
	mac.addr[5] = (EMAC_SA1H >> 8) & 0xff;
	mac.addr[4] = (EMAC_SA1H >> 0) & 0xff;
	mac.addr[3] = (EMAC_SA1L >> 24) & 0xff;
	mac.addr[2] = (EMAC_SA1L >> 16) & 0xff;
	mac.addr[1] = (EMAC_SA1L >> 8) & 0xff;
	mac.addr[0] = (EMAC_SA1L >> 0) & 0xff;
	return mac;
}
...
static void emac_reset(void)
{
	...
	uint32_t phy_id = phy_hw_read(NIC_PHY_ID1) << 16 | phy_hw_read(NIC_PHY_ID2);
...
static void emac_start(void)
{
	...
	tx_buf_tab[0].stat = TXS_USED | TXS_WRAP;
...
static void eth_waitTxReady(void)
{
	...
	while (!(tx_buf_tab[0].stat & TXS_USED))
		event_wait(&send_wait);
}

void eth_send(const struct EthernetDescriptor *list, size_t count)
{
	ASSERT(count <= EMAC_TX_DESCRIPTORS);

	eth_waitTxReady();

	for (size_t i = 0; i < count; i++)
	{
		ASSERT(list[i].len < 2048);
		tx_buf_tab[i].addr = (uint32_t)list[i].data;
		tx_buf_tab[i].stat = list[i].len;
	}

	tx_buf_tab[count - 1].stat |= (TXS_LAST_BUFF | TXS_WRAP);
...
static ssize_t eth_getFrame(uint8_t *buf, size_t len)
{
	uint8_t *addr;
	size_t rd_len = 0;
...
INLINE volatile BufDescriptor *DESC_NEXT(volatile BufDescriptor *desc)
{
	if (++desc >= &rx_buf_tab[EMAC_RX_BUFFERS])
		desc = &rx_buf_tab[0];
	return desc;
}

#define DESC_RESET(desc)                  \
	do                                    \
	{                                     \
		(desc)->addr &= ~RXBUF_OWNERSHIP; \
		(desc)->stat = 0;                 \
	} while (0)

#define DESC_OWN(desc) ((desc)->addr & RXBUF_OWNERSHIP)

#define DESC_SOF(desc) \
	(DESC_OWN(desc) && ((desc)->stat & RXS_SOF))

#define DESC_EOF(desc) \
	(DESC_OWN(desc) && ((desc)->stat & RXS_EOF))

#define DESC_LEN(desc) \
	((desc)->stat & RXS_LENGTH_FRAME)

size_t eth_recv(struct EthernetDescriptor *list, size_t count)
{
#if 1

	size_t len = eth_getFrameLen();
	size_t rd_len = 0;
	struct EthernetDescriptor *end = list + count;
	while (len)
	{
		if (list >= end)
		{
			LOG_ERR("Not enough space in ethernet descriptors!\n");
			break;
		}

		size_t size = MIN(len, list->len);
		eth_getFrame(list->data, size);
...
#else
	/* New version, to be debugged! */

	static volatile BufDescriptor *curr_desc = &rx_buf_tab[0];
	volatile BufDescriptor *start;
	volatile BufDescriptor *end;
	volatile BufDescriptor *first;
	end = start = NULL;

	do
	{
		/* Find start and end descriptors for this frame */
		for (first = NULL; !end; curr_desc = DESC_NEXT(curr_desc))
		{
			/* If curr_desc points again to where we started there is no frame to receive */
			if (first == curr_desc)
				break;
			...
			if (!start)
			{
				if (DESC_SOF(curr_desc))
					start = curr_desc;
				else
				{
					DESC_RESET(curr_desc);
					continue;
				}
			}

			if (!DESC_OWN(curr_desc) || (DESC_SOF(curr_desc) && curr_desc != start))
			{
				DESC_RESET(start);
				curr_desc = start;
				start = NULL;
	...
	size_t len = DESC_LEN(end);
	size_t len_desc = EMAC_RX_BUFSIZ;
	size_t off_eth = 0;
	size_t off_desc = 0;
	EthernetDescriptor *curr_eth = list;
	EthernetDescriptor *end_eth = list + count;
	size_t rd_len = 0;

	/* Fill the higher level ethernet descriptors */
	while (len)
	{
		if (curr_eth >= end_eth)
		{
		...
		size_t space_eth = curr_eth->len - off_eth;
		size_t space_desc = len_desc - off_desc;
		size_t size = MIN(space_eth, space_desc);
		memcpy(((uint8_t *)curr_eth->data) + off_eth,
		       ((uint8_t *)curr_desc->addr) + off_desc, size);

		off_eth += size;
		if (off_eth >= curr_eth->len)
		{
			curr_eth++;
			off_eth = 0;
		}
		...
		off_desc += size;
		if (off_desc >= EMAC_RX_BUFSIZ)
		{
			DESC_RESET(curr_desc);
			curr_desc = DESC_NEXT(curr_desc);
			off_desc = 0;
			len_desc = MIN((size_t)EMAC_RX_BUFSIZ, len);
		}
