...
#define STM32_ETH_GPIO_CNT 9
typedef struct Stm32EthCfg
{
	struct EthCfg e;
	Stm32EthPinCfg pins[STM32_ETH_GPIO_CNT];
	Stm32MacMode mac_mode;
} Stm32EthCfg;

#define ETH_STM32 MAKE_ID('E', 'S', 'T', '3')

INLINE const Stm32EthCfg *ETHSTM32_CAST(const EthCfg *cfg)
{
	ASSERT(cfg->_type == ETH_STM32);
	return (const Stm32EthCfg *)cfg;
}
