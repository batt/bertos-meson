...
static int usb_ep_get_stall(int EP, bool *pStall)
{
	...
	*pStall = (EP & 0x01) ? (ep_ctrl_get_stat_tx(ep_cnfg[EP].hw) == EP_STALL) : /* IN EP  */
...
static void usb_status_handler(UNUSED_ARG(int, EP))
{
	...
	else
	{
		...
		ep_cnfg[CTRL_ENP_IN].complete =
		    (setup_packet.bRequest == USB_REQ_SET_ADDRESS) ? usb_add_status_handler_end : usb_status_handler_end;
	}
}
                                ...
                                ticks_t timeout)
{
	...
	else if (!event_waitTimeout(&usb_event_done[ep_num >> 1], timeout))
		return 0;
