...
int __check_wall(long *wall, int size, const char *name, const char *file, int line);
	...
	#define ASSERT_VALID_PTR(p) (IS_VALID_PTR(p) \
		                             ? 0         \
		                             : __invalid_ptr(p, #p, THIS_FILE, __LINE__))
	 ...
	 */
	#define ASSERT_VALID_PTR_OR_NULL(p) ((void)(LIKELY((p == NULL) || ((void *)(p) >= (void *)CPU_RAM_START)) \
		                                            ? 0                                                       \
		                                            : __invalid_ptr((p), #p, THIS_FILE, __LINE__)))
	 ...
	 * \{
	 */
	#define NEW_INSTANCE(CLASS)  \
		do                       \
		{                        \
			++CLASS::__instances \
		} while (0)
	#define DELETE_INSTANCE(CLASS) \
		do                         \
		{                          \
			--CLASS::__instances   \
		} while (0)
...
INLINE void TRACEMSG(UNUSED_ARG(const char *, msg), ...)
{
	...
	#define ASSERT_ZERO_INSTANCES(CLASS) \
		do                               \
		{                                \
		} while (0)
...
INLINE void kputchar(UNUSED_ARG(char, c))
{ /* nop */
}
INLINE int kputnum(UNUSED_ARG(int, num)) { return 0; }
INLINE void kputs(UNUSED_ARG(const char *, str))
{ /* nop */
}
INLINE void kdump(UNUSED_ARG(const void *, buf), UNUSED_ARG(size_t, len))
{ /* nop */
}
	...
	#else
/* ...but GCC can. */
INLINE void kprintf(UNUSED_ARG(const char *, fmt), ...)
{ /* nop */
}
	#endif
...
#else
INLINE bool klogger_init(void)
{
	return true;
}
INLINE void klogger_exit(void)
{ /* nop */
}
