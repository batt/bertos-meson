...
extern uint32_t log_cnt;
...
#else /* CONFIG_NET_LOG != 0 */
	#if LOG_FORMAT == LOG_FMT_VERBOSE
		#define LOG_PRINT(str_level, str, ...) log_printf("<182>%d-%s():%d:%s: " str, log_cnt, __func__, __LINE__, str_level, ##__VA_ARGS__)
	#elif LOG_FORMAT == LOG_FMT_TERSE
		#define LOG_PRINT(str_level, str, ...) log_printf("<182>%d-%s: " str, log_cnt, str_level, ##__VA_ARGS__)
	#else
		#error No LOG_FORMAT defined
	#endif
	...
	#define LOG_ERRB(x) x
#else
INLINE void LOG_ERR(UNUSED_ARG(const char *, fmt), ...)
{ /* nop */
}
...
#else
INLINE void LOG_WARN(UNUSED_ARG(const char *, fmt), ...)
{ /* nop */
}
...
#else
INLINE void LOG_INFO(UNUSED_ARG(const char *, fmt), ...)
{ /* nop */
}
