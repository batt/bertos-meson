		...
		for (;;)
		{
			...
			do
			{
				...
				else
				{
					...
					PGM_FUNC(text_xprintf)
					(lev->bitmap, 1, 3, 0, PGM_STR("%d"), *lev->ch1_val);
					PGM_FUNC(text_xprintf)
					(lev->bitmap, 1, 14, 0, PGM_STR("%d"), *lev->ch2_val);
			...
			if (keys & (K_UP | K_DOWN))
			{
				if (keys & K_UP)
				{
					...
					if (chn != 0 ||
					    (*lev->ch1_val + step <= lev->max && *lev->ch2_val + step <= lev->max))
					{
				...
				{
					if (chn != 0 ||
					    (*lev->ch1_val - step >= lev->min && *lev->ch2_val - step >= lev->min))
					{
						if (chn != 2)
						{
							*lev->ch1_val -= step;
		...
		for (;;)
		{
			...
			do
			{
				...
				else
				{
					...
					PGM_FUNC(text_xyprintf)
					(lev->bitmap, 0, bar1.y1 - lev->bitmap->font->height,
