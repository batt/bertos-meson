...
ifeq ($$($(1)_HOSTED),1)
        ...
        $$(OUTDIR)/$(1).tgt : $$(OUTDIR)/$(1).s19 $$(OUTDIR)/$(1).hex $$(OUTDIR)/$(1).bin $$(OUTDIR)/$(1).czf $$(OUTDIR)/$(1)-update.exe $$(OUTDIR)/$(1)-kk0000.czf $$(OUTDIR)/$(1)-generic-update.exe
        #$$(OUTDIR)/$(1).tgt : $$(OUTDIR)/$(1).bin $$(OUTDIR)/$(1).czf $$(OUTDIR)/$(1)-update.exe
endif
...
$$(OUTDIR)/$(1).czf: $$(OUTDIR)/$(1).bin
	$L "$(1): Creating .czf file"
	$Q $(CZF) $$< $$@ $(1)

$$(OUTDIR)/$(1)-kk0000.czf: $$(OUTDIR)/$(1).bin
	$L "$(1): Creating generic .czf file"
	$Q $(CZF) $$< $$@ kk0000

$$(OUTDIR)/$(1)-update.exe: $$(OUTDIR)/$(1).czf
	$L "$(1): Creating autoupdater exe file"
	$Q if [ ! -z "$$($(1)_UPDIP)" ] ; then \
		$(UPDATER) $$< $$@ `$(FINDREV) $(1) official` --ip=$$($(1)_UPDIP) $$($(1)_UPDRESET) --cpu=$$($(1)_UPDCPU); \
		cp $$@ $$(OUTDIR)/$(1)-update-`$(FINDREV) $(1) official`.exe; \
	fi

$$(OUTDIR)/$(1)-generic-update.exe: $$(OUTDIR)/$(1)-kk0000.czf
	$L "$(1): Creating generic autoupdater exe file"
	$Q if [ ! -z "$$($(1)_UPDIP)" ] ; then \
		$(UPDATER) $$< $$@ `$(FINDREV) $(1) official` --ip=$$($(1)_UPDIP) $$($(1)_UPDRESET) --cpu=$$($(1)_UPDCPU); \
		cp $$@ $$(OUTDIR)/$(1)-generic-update-`$(FINDREV) $(1) official`.exe; \
	fi
