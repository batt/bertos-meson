...
static bool readHdr(struct BattFsSuper *disk, pgcnt_t page, struct BattFsPageHeader *hdr)
{
	 ...
	 * resides at page end.
	 */
	if (kblock_read(disk->dev, page, buf, disk->data_size, BATTFS_HEADER_LEN) != BATTFS_HEADER_LEN)
	{
		LOG_ERR("page[%d]\n", page);
		return false;
	}
...
#warning FIXME:refactor computeFcs to save time and stack
	 ...
	 * resides at page end.
	 */
	if (kblock_write(disk->dev, page, buf, disk->data_size, BATTFS_HEADER_LEN) != BATTFS_HEADER_LEN)
	{
...
static size_t battfs_write(struct KFile *fd, const void *_buf, size_t size)
{
	...
	if (fd->seek_pos > fd->size)
	{
		 ...
		 * of the file.
		 */
		if (kblock_buffered(disk->dev) && ((fdb->start[fdb->max_off] != kblock_cachedBlock(disk->dev)) || !kblock_cacheDirty(disk->dev)))
		{
